...
# --------------------------
# Attention kernels（可替换为你的三种算子）
# q/k/v: [B, nH, S, d] -> out: [B, nH, S, d]
# --------------------------
def attention_kernel_a(q, k, v, attn_mask=None, dropout_p=0.0, is_causal=True):
    # 基线A：PyTorch SDPA
...
def attention_kernel_b(q, k, v, attn_mask=None, dropout_p=0.0, is_causal=True):
    # 基线B：显式 QK^T -> softmax -> AV
    d = q.size(-1)
    attn_scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(d)  # [B, nH, S, S]
...
def attention_kernel_c(q, k, v, attn_mask=None, dropout_p=0.0, is_causal=True):
    # 基线C：示例为“带掩码的 SDPA（禁用 is_causal，显式传入 causal mask）”
    # 你可以将其替换为第三个自研/融合内核，接口保持一致即可。
    d = q.size(-1)
    B, nH, S_q, _ = q.shape
    Bk, nHk, S_k, _ = k.shape
    # 显式构造下三角 mask（float型，0/ -inf），兼容不等长 S_q, S_k
    i = torch.arange(S_q, device=q.device).unsqueeze(-1)
    j = torch.arange(S_k, device=q.device).unsqueeze(0)
    causal = (j > i)
    causal_mask = torch.zeros((S_q, S_k), device=q.device, dtype=q.dtype)  # same dtype as q for SDPA
    causal_mask = causal_mask.masked_fill(causal, float("-inf"))
    return F.scaled_dot_product_attention(
        q, k, v,
        attn_mask=causal_mask if attn_mask is None else attn_mask,
        dropout_p=dropout_p if training_mode() else 0.0,
        is_causal=False  # 使用显式掩码
    )
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, kernel="A", attn_mask=None, is_causal=True):
        B, S, H = x.shape
        ...
        kernel = kernel.upper()
        if kernel == "A":
            out = attention_kernel_a(q, k, v, attn_mask=attn_mask, dropout_p=self.attn_dropout, is_causal=is_causal)
        elif kernel == "B":
            out = attention_kernel_b(q, k, v, attn_mask=attn_mask, dropout_p=self.attn_dropout, is_causal=is_causal)
        elif kernel == "C":
            out = attention_kernel_c(q, k, v, attn_mask=attn_mask, dropout_p=self.attn_dropout, is_causal=is_causal)
        else:
...
# --------------------------
# 单层 Transformer（Qwen-3 Dense 风格：Pre-RMSNorm + MHA + 残差 + Pre-RMSNorm + 门控MLP + 残差）
# --------------------------
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, kernel="A", attn_mask=None, is_causal=True):
        h = self.norm1(x)
        attn_out = self.attn(h, kernel=kernel, attn_mask=attn_mask, is_causal=is_causal)
        x = x + attn_out
        h2 = self.norm2(x)
        x = x + self.mlp(h2)
        return x
...
# --------------------------
@torch.no_grad()
def benchmark_infer(block, x, kernel, steps_warmup, steps_measure, dev):
    for _ in range(steps_warmup):
        _ = block(x, kernel=kernel)
    sync_device(dev)
...
# --------------------------
def benchmark_train(block, x, kernel, steps_warmup, steps_measure, optimizer, dev, use_amp=True):
    block.train()
    loss_fn = nn.MSELoss()
...
def main():
    ...
    if not cfg.run_backward:
        ...
        results_infer = {}
        for k in kernels:
            mean, std = benchmark_infer(block, x, kernel=k,
                                        steps_warmup=cfg.warmup_steps,
                                        steps_measure=cfg.measure_steps, dev=dev)
            results_infer[k] = (mean, std)
        print("\n===== Inference (Prefill) =====")
        for k in kernels:
            m, s = results_infer[k]
            label = {"A": "SDPA", "B": "Ref", "C": "SDPA+mask"}[k]
            print(f"Kernel {k} ({label}) : mean={m:.3f} ms, std={s:.3f} ms")
        # 找更快者
        best_k = min(results_infer.items(), key=lambda kv: kv[1][0])[0]
        print(f"Faster kernel   : {best_k}")
    else:
        ...
        for idx, k in enumerate(kernels):
            if idx == 0:
                ...
                optimizer = torch.optim.AdamW([p for p in block.parameters() if p.requires_grad],
            ...
            results_train_fwd[k] = (fwd_mean, fwd_std)
            results_train_step[k] = (step_mean, step_std)

        print("\n===== Training (Prefill) =====")
        print("Forward only (ms/iter):")
        for k in kernels:
            m, s = results_train_fwd[k]
            label = {"A": "SDPA", "B": "Ref", "C": "SDPA+mask"}[k]
            print(f"  Kernel {k} ({label}): mean={m:.3f}, std={s:.3f}")
        best_fwd = min(results_train_fwd.items(), key=lambda kv: kv[1][0])[0]
        print(f"  Faster forward: {best_fwd}")

        print("\nForward+Backward+Step (ms/iter):")
        for k in kernels:
            m, s = results_train_step[k]
            label = {"A": "SDPA", "B": "Ref", "C": "SDPA+mask"}[k]
            print(f"  Kernel {k} ({label}): mean={m:.3f}, std={s:.3f}")
        best_step = min(results_train_step.items(), key=lambda kv: kv[1][0])[0]
        print(f"  Faster end2end: {best_step}")
