...
def main():
    ...
    if not cfg.run_backward:
        block.eval()
        mean_a, std_a = benchmark_infer(block, x, kernel="A",
                                        steps_warmup=cfg.warmup_steps, steps_measure=cfg.measure_steps, dev=dev)
        mean_b, std_b = benchmark_infer(block, x, kernel="B",
                                        steps_warmup=cfg.warmup_steps, steps_measure=cfg.measure_steps, dev=dev)
        print("\n===== Inference (Prefill) =====")
        print(f"Kernel A (SDPA) : mean={mean_a:.3f} ms, std={std_a:.3f} ms")
        print(f"Kernel B (Ref)  : mean={mean_b:.3f} ms, std={std_b:.3f} ms")
        faster = "A" if mean_a < mean_b else "B"
        ratio = (max(mean_a, mean_b) / max(1e-9, min(mean_a, mean_b))) - 1.0
        print(f"Faster kernel   : {faster} (~{ratio*100:.1f}% faster)")
    else:
        ...
        (fwd_mean_a, fwd_std_a), (step_mean_a, step_std_a) = benchmark_train(
            block, x, kernel="A",
        ...
        (fwd_mean_b, fwd_std_b), (step_mean_b, step_std_b) = benchmark_train(
            block, x, kernel="B",
            steps_warmup=cfg.warmup_steps, steps_measure=cfg.measure_steps,
            optimizer=optimizer, dev=dev, use_amp=use_amp
        )
        ...
        print(f"  Kernel A: mean={fwd_mean_a:.3f}, std={fwd_std_a:.3f}")
        print(f"  Kernel B: mean={fwd_mean_b:.3f}, std={fwd_std_b:.3f}")
        faster_fwd = "A" if fwd_mean_a < fwd_mean_b else "B"
        ratio_fwd = (max(fwd_mean_a, fwd_mean_b) / max(1e-9, min(fwd_mean_a, fwd_mean_b))) - 1.0
        print(f"  Faster forward: {faster_fwd} (~{ratio_fwd*100:.1f}% faster)")

        print("\nForward+Backward+Step (ms/iter):")
        print(f"  Kernel A: mean={step_mean_a:.3f}, std={step_std_a:.3f}")
        print(f"  Kernel B: mean={step_mean_b:.3f}, std={step_std_b:.3f}")
        faster_step = "A" if step_mean_a < step_mean_b else "B"
        ratio_step = (max(step_mean_a, step_mean_b) / max(1e-9, min(step_mean_a, step_mean_b))) - 1.0
        print(f"  Faster end2end: {faster_step} (~{ratio_step*100:.1f}% faster)")
