...
def run_benchmark(version, B, H, T, D, warmup_runs, num_runs):
    ...
    # We remove `torch.no_grad()` and add `requires_grad=True`
    ...
    # Create a dummy gradient for the output to start the backward pass
    ...
    print(f"\nStarting timer for {num_runs} runs (forward + backward)...")
    ...
    if device == 'cuda':
        ...
        # Run the operation once to measure its peak memory
        run_op()
    ...
    # Run forward pass one last time to get the output shape for the log
    with torch.no_grad():
        if version in ['chunk', 'fused_chunk']:
            output = target_func(q, k, v, g, scale=1.0, output_final_state=final_state)
        else: # parallel
